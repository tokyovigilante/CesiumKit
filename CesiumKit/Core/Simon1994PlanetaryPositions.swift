...
*/
struct Simon1994PlanetaryPositions {
    ...
    *--------------------------------------------------------------------*/
    ...
    static func computeTdbMinusTtSpice(daysSinceJ2000InTerrestrialTime: Double) -> Double {
        let g = 6.239996 + (0.0172019696544) * daysSinceJ2000InTerrestrialTime
        return 1.657e-3 * sin(g + 1.671e-2 * sin(g))
    }
    
    private static let TdtMinusTai = 32.184
    private static let J2000d = 2451545.0
    
    private static func taiToTdb(date: NSDate) -> NSDate {
        //Converts TAI to TT
        var result = date.dateByAddingTimeInterval(TdtMinusTai)
        
        //Converts TT to TDB
        let days = result.totalDays() - J2000d
        result = result.dateByAddingTimeInterval(computeTdbMinusTtSpice(days))
        
        return result
    }
    
    private static let epoch =  NSDate(julianDayNumber: 2451545, secondsOfDay: 0)//, TimeStandard.TAI); //Actually TDB (not TAI)
    ...
    */
    */
    static func computeSimonEarthMoonBarycenter(date: NSDate) -> Cartesian3 {
    ...
    */
    func computeSimonMoon(date: NSDate) -> Cartesian3 {
    /*var scratchDate = Simon1994PlanetaryPositions.taiToTdb(date).computeJulianDateComponents()
    let x = (scratchDate.dayNumber - epoch.dayNumber) + ((scratchDate.secondsOfDay - epoch.secondsOfDay)/TimeConstants.SECONDS_PER_DAY);
     ...
     */
    private static let _moonEarthMassRatio = 0.012300034 // From 1992 mu value in Table 2
    private static let factor = _moonEarthMassRatio / (_moonEarthMassRatio + 1.0) * -1
    
    func computeSimonEarth(date: NSDate) -> Cartesian3 {
        let result = computeSimonMoon(date)
        //return result.multiplyByScalar(factor)
        return Cartesian3()
    }
    /*
    ...
    */
    private static let _axesTransformation = Matrix3(
    ...
    */
    static func computeSunPositionInEarthInertialFrame (date: NSDate = NSDate()) -> Cartesian3 {
